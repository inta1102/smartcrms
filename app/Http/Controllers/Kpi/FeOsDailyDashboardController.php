...
class FeOsDailyDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // baseline snapshot month fallback
        ...
        foreach ($labels as $d) {
            ...
            $series['os_potensi'][]    = $byDate[$d]['os_potensi'] ?? null;
            $series['os_kl'][]         = $byDate[$d]['os_kl'] ?? null;
            $series['rr'][]            = $byDate[$d]['rr'] ?? null;
            $series['pct_lt'][]        = $byDate[$d]['pct_lt'] ?? null;
            $series['pct_dpk'][]       = $byDate[$d]['pct_dpk'] ?? null;
            $series['pct_potensi'][]   = $byDate[$d]['pct_potensi'] ?? null;
            $series['pct_kl'][]        = $byDate[$d]['pct_kl'] ?? null;
        }
        ...
        // =============================
        // FE tambahan: DPK EOM -> Potensi hari ini
        // =============================
        ...
        // =============================
        // FE tambahan: Potensi EOM -> KL hari ini
        // =============================
        ...
        // =============================
        // LIST FE: DPK EOM -> Potensi hari ini
        // =============================
        ...
        // =============================
        // LIST FE: Potensi EOM -> KL hari ini
        // =============================
        ...
        // =============================
        // TABEL 2) COHORT LT EOM -> status hari ini
        // =============================
        ...
        $isLtOnly = function ($r) use ($isDpk, $isPotensi, $isKl, $isL0) {
            ...
            if ($isPotensi($r)) return false;
            if ($isKl($r)) return false;
        ...
        // =============================
        // TABEL 2A) COHORT L0 EOM -> status posisi terakhir
        // =============================
        $today = now()->toDateString();
        $uid   = (int) auth()->id();
