...
class MarketingKpiSheetController
{
    public function index(Request $request)
    {
        ...
        // $role = $request->query('role', 'SO');


        // role: AO atau SO (default AO)
        $role = strtoupper((string)($request->input('role') ?: 'AO'));
        if (!in_array($role, ['AO', 'SO'], true)) $role = 'AO';

        if ($role === 'SO') {
            // ============================
            // SO SHEET
            // Bobot sesuai contoh kamu:
            // OS 55% | NOA 15% | RR 20% | Komunitas 10%
            // ============================
            $weights = [
                'os'       => 0.55,
                'noa'      => 0.15,
            ...
            $rows = DB::table('kpi_so_monthlies as m')
                ...
                ->leftJoin('kpi_so_targets as t', function ($j) use ($period) {
            ...
            // hitung pencapaian + PI per baris (biar blade simpel)
            $items = $rows->map(function ($r) use ($weights) {
                ...
                // RR target biasanya 100% (di excel kamu), tapi kalau target gak ada: tampilkan saja rr_pct
                $targetRr = 100;
        ...
        // ============================
        // AO SHEET
        // Bobot AO (sesuai service kamu):
        // OS 35% | NOA 15% | RR 25% | Kolek(NPL Mig) 15% | Activity 10%
        // NOTE: RR AO sudah kamu ubah definisinya jadi "OS lancar / total OS"
        // dan harusnya rr_pct di kpi_ao_monthlies sudah terisi dari service baru.
        // ============================
        $weights = [
            'os'       => 0.35,
            'noa'      => 0.15,
        ...
        $rows = DB::table('kpi_ao_monthlies as m')
            ->join('users as u', 'u.id', '=', 'm.user_id')
            ->leftJoin('kpi_ao_targets as t', function ($j) use ($period) {
                $j->on('t.user_id', '=', 'm.user_id')
                  ->where('t.period', '=', $period);
            })
            ->where('m.period', $period)
