...
<div class="mx-auto w-full max-w-6xl space-y-5 px-4 py-6">
    ...
    <div class="rounded-2xl border border-slate-100 bg-white px-4 py-3 shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            @php
                $hasAssessment = filled($case->assessment);
            @endphp

            <div x-data="{ open:false }" class="flex flex-col items-end gap-2 text-xs">
                {{-- Sync legacy --}}
                ...
                @can('updateAssessment', $case)
                    ...
                    <button
                        ...
                        x-on:click="
                            open = !open;
                            $nextTick(() => { if(open) document.getElementById('assessmentTextarea')?.focus(); })
                        "
                    >
                        {{ $hasAssessment ? 'Update Assessment' : 'Add Assessment' }}
                    </button>

                    {{-- Panel input assessment --}}
                    <div
                        x-cloak
                        x-show="open"
                        x-transition
                        class="w-[520px] max-w-full rounded-xl border border-slate-200 bg-white p-4 shadow-sm"
                    >
                        <form method="POST" action="{{ route('cases.assessment.store', $case) }}" class="space-y-3">
                            @csrf

                            <div>
                                <label class="mb-1 block text-sm font-semibold text-slate-700">Assessment</label>
                                <textarea
                                    id="assessmentTextarea"
                                    name="assessment"
                                    rows="5"
                                    class="w-full rounded-lg border border-slate-300 p-3 text-sm focus:border-blue-500 focus:outline-none"
                                    placeholder="Tulis assessment kasus: kondisi debitur, rencana penagihan, mitigasi, next action, dll."
                                >{{ old('assessment', $case->assessment) }}</textarea>
        ...
        @if(filled($case->assessment))
            <div class="mt-4 border rounded-xl p-4 bg-white">
                <div class="flex items-start justify-between gap-4">
                <div>
                    <div class="font-semibold">Assessment</div>
                    <div class="text-xs text-gray-500 mt-1">
                    Terakhir update:
                    {{ optional($case->assessment_updated_at)->format('d-m-Y H:i') ?? '-' }}
                    @if($case->assessment_updated_by)
                        • oleh: {{ optional($case->assessmentUpdater)->name ?? ('User#'.$case->assessment_updated_by) }}
                    @endif
                    </div>
                </div>
                </div>
