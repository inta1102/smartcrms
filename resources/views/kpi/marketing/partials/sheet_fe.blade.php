...
{{-- ==========================================================
   NOTE:
   ✅ HAPUS include partial _tlfe_recap karena itu sumber "output nyasar"
   TL recap sudah dirender utuh di bawah.
   ========================================================== --}}
...
@if(!empty($tlRecap))
  @php
    ...
    // ✅ infoPct khusus TL recap (fallback aman)
    $infoPct = (float)($tlRecap->nett_os_down_pct_info ?? 0);

    // kalau belum ada pct info, kita hitung sederhana: nett_os_down / os_kol2_awal
    $osAwal = (float)($tlRecap->os_kol2_awal ?? 0);
    $nett   = (float)($tlRecap->nett_os_down ?? 0);
    if ($infoPct <= 0 && $osAwal > 0) {
        $infoPct = ($nett / $osAwal) * 100;
    }

    // ✅ baseline untuk TL recap
    $baselineOk = true;

    // kalau kamu simpan baseline_ok di tlRecap
    if (isset($tlRecap->baseline_ok)) {
        $baselineOk = ((int)$tlRecap->baseline_ok === 1);
    }
  @endphp
  ...
  <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
    ...
    <div class="px-5 py-4 border-b border-slate-200 flex flex-col md:flex-row md:items-start md:justify-between gap-4">
      ...
      @php
        ...
        $lr  = $tlRecap->leader_role ?? 'TLFE';

        // weights (fallback aman)
        $wOs  = (float)($wOs  ?? ($weights['os_turun'] ?? 0.40));
        $wMg  = (float)($wMg  ?? ($weights['migrasi']  ?? 0.40));
        $wPen = (float)($wPen ?? ($weights['penalty']  ?? 0.20));

        // baseline & info pct
        $baselineOk = (int)($tlRecap->baseline_ok ?? 1) === 1;
        $infoPct    = (float)($tlRecap->nett_os_down_pct_info ?? 0);

        // migrasi TL recap (ambil langsung dari tlRecap, jangan variabel liar)
    ...
    <div class="p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div class="text-sm font-bold text-slate-900">
          Murni: {{ $fmtRp($tlRecap->nett_os_down ?? 0) }}<br>
          Migrasi: {{ $fmtRp($tlRecap->nett_os_down_migrasi ?? 0) }}<br>
          Total turun: {{ $fmtRp($tlRecap->nett_os_down_total ?? 0) }}
        </div>
        <div class="text-xs text-slate-500 mt-1">
          Nett/OS awal: {{ $fmtPct($infoPct, 2) }}
        </div>
      </div>
    </div>
    ...
    <div class="px-4 pb-4 overflow-x-auto">
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-200">
          ...
          <tr>
            ...
            <td class="px-3 py-2 text-right">
              ...
              <div class="text-xs text-slate-500 mt-1 space-y-0.5">
                ...
                <div>Total turun: <b>{{ $fmtRp($tlRecap->nett_os_down_total ?? 0) }}</b></div>
                <div>Turun karena migrasi: <b>{{ $fmtRp($tlRecap->nett_os_down_migrasi ?? 0) }}</b></div>
              </div>
            </td>
            <td class="px-3 py-2 text-right">
          ...
          {{-- MIGRASI (reverse) --}}
          @php
            // ====== HARDEN DEFAULTS (anti undefined variable) ======
            $items   = $items ?? collect();
            $it      = $it ?? null;
            ...
            // weights default
            $weights = $weights ?? ['os_turun'=>0.40,'migrasi'=>0.40,'penalty'=>0.20];

            // helper fallback (kalau belum ada)
            $fmtPct = $fmtPct ?? fn($v,$d=2)=> number_format((float)($v??0), $d, ',', '.') . '%';
            $fmtRp  = $fmtRp  ?? fn($v)=> 'Rp ' . number_format((float)($v??0), 0, ',', '.');

            // ====== TL MIGRASI NUMBERS (default aman) ======
            $tlTargetMg = (float)($tlTargetMg ?? data_get($tlRecap, 'target_npl_migration_pct', 0));
            $tlActualMg = (float)($tlActualMg ?? data_get($tlRecap, 'npl_migration_pct', 0));

            // ====== STATUS TEXT (default aman) ======
            $vsText = $vsText ?? function($actual, $target) {
                $a = (float)($actual ?? 0);
                $t = (float)($target ?? 0);

                // reverse metric: lebih kecil lebih baik
                if ($t <= 0) return 'OK';
                if ($a <= $t) return 'OK';
                $pp = $a - $t;
                $x  = $t > 0 ? ($a / $t) : 0;
                return 'ALERT • +' . number_format($pp,2) . 'pp • ' . number_format($x,1) . 'x';
            };

            // ====== CLASS BADGE MIGRASI (default aman) ======
            $tlMgClass = $tlMgClass ?? (
                ($tlTargetMg > 0 && $tlActualMg <= $tlTargetMg)
