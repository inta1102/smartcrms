  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
    @for($i=0; $i<3; $i++)
      ...
      <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        @if($r)
          @if($tab==='score')
            <div class="mt-1 text-lg font-bold text-slate-900">{{ $r->user?->name ?? '-' }}</div>
            <div class="text-xs text-slate-500">AO Code: <b>{{ $r->user?->ao_code ?? '-' }}</b></div>

            <div class="mt-3 grid grid-cols-2 gap-2">
              <div class="rounded-xl bg-slate-50 p-2">
                <div class="text-[11px] text-slate-500">Total Score</div>
                <div class="font-bold">{{ number_format((float)$r->score_total,2) }}</div>
              </div>
              <div class="rounded-xl bg-slate-50 p-2">
                <div class="text-[11px] text-slate-500">OS Growth</div>
                <div class="font-bold">Rp {{ number_format((int)$r->os_growth,0,',','.') }}</div>
              </div>
            </div>
          @else
            {{-- Growth tab: object hasil query tidak punya user relation --}}
            <div class="mt-1 text-lg font-bold text-slate-900">{{ $r->ao_name ?? ('AO '.$r->ao_code) }}</div>
            ...
            <div class="mt-3 grid grid-cols-2 gap-2">
              ...
              <div class="rounded-xl bg-slate-50 p-2">
                ...
                <div class="font-bold">{{ number_format((int)$r->noa_growth) }}</div>
              </div>
            </div>
          @endif
        @else
      ...
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          <div class="font-bold text-slate-900 truncate">
            #{{ $r->rank }} - {{ $r->user?->name ?? $r->ao_name ?? '-' }}
          </div>
          <div class="text-xs text-slate-500">
            AO Code:
            <b>{{ $r->user?->ao_code ?? $r->ao_code ?? '-' }}</b>
          </div>
        </div>
          ...
          @forelse($rows as $r)
            @if($tab==='score')
              <tr class="border-b hover:bg-slate-50">
                ...
                <td class="px-4 py-3">
                  ...
                  <div class="text-xs text-slate-500">AO Code: {{ $r->user?->ao_code ?? '-' }}</div>
                </td>
                <td class="px-4 py-3 text-right">Rp {{ number_format((int)$r->os_growth,0,',','.') }}</td>
                <td class="px-4 py-3 text-right">{{ number_format((int)$r->noa_growth) }}</td>
                <td class="px-4 py-3 text-right">{{ number_format((float)$r->score_os,2) }}</td>
                <td class="px-4 py-3 text-right">{{ number_format((float)$r->score_noa,2) }}</td>
                <td class="px-4 py-3 text-right font-bold text-slate-900">{{ number_format((float)$r->score_total,2) }}</td>
            ...
            @else
              <tr class="border-b hover:bg-slate-50">
                <td class="px-4 py-3 font-bold text-slate-900">{{ $r->rank }}</td>
                <td class="px-4 py-3">
                  <div class="font-semibold text-slate-900">{{ $r->ao_name ?? ('AO '.$r->ao_code) }}</div>
                  <div class="text-xs text-slate-500">AO Code: {{ $r->ao_code }}</div>
                </td>
                <td class="px-4 py-3 text-right">Rp {{ number_format((int)$r->os_prev,0,',','.') }}</td>
                <td class="px-4 py-3 text-right">Rp {{ number_format((int)$r->os_now,0,',','.') }}</td>
                <td class="px-4 py-3 text-right font-semibold">Rp {{ number_format((int)$r->os_growth,0,',','.') }}</td>
                <td class="px-4 py-3 text-right">{{ number_format((int)$r->noa_prev) }}</td>
                <td class="px-4 py-3 text-right">{{ number_format((int)$r->noa_now) }}</td>
                <td class="px-4 py-3 text-right font-semibold">{{ number_format((int)$r->noa_growth) }}</td>
              </tr>
            @endif
          @empty
            <tr>
              <td colspan="{{ $tab==='score' ? 8 : 8 }}" class="px-4 py-6 text-center text-slate-500">
                Belum ada data ranking untuk periode ini.
              </td>
            </tr>
          @endforelse
