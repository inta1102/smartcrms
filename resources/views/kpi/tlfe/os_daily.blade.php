...
@section('title', 'Dashboard FE - OS Harian')
...
@php
  ...
  // L0 : selain itu
  // ========= bucket helper FE only (L0 / LT / DPK / POTENSI / KL) =========
  $bucketFromFt = function ($ftPokok, $ftBunga, $kolek = null) {
      $fp = (int)($ftPokok ?? 0);
      $fb = (int)($ftBunga ?? 0);
      $k  = (int)($kolek ?? 0);

      if ($k === 3) return 'KL';
      if ($k === 2 && ($fp === 3 || $fb === 3)) return 'POTENSI';
      if ($k === 2 && ($fp === 2 || $fb === 2)) return 'DPK';
      if ($k === 1 && ($fp === 1 || $fb === 1)) return 'LT';
      if ($k === 1 && $fp === 0 && $fb === 0) return 'L0';
  ...
  $progressText = function ($r) use ($bucketFromFt) {
      $hasPrev = isset($r->prev_ft_pokok) || isset($r->prev_ft_bunga) || isset($r->prev_bucket) || isset($r->prev_kolek);
      if (!$hasPrev) return '-';
  ...
  $progressBadgeClass = function (string $prog) {
      ...
      // worsening FE
      if (
          str_contains($prog, 'L0→LT') ||
          str_contains($prog, 'LT→DPK') ||
          str_contains($prog, 'DPK→POTENSI') ||
          str_contains($prog, 'POTENSI→KL') ||
          str_contains($prog, 'L0→DPK')
      ) {
      ...
      // improving
      if (
          str_contains($prog, 'DPK→LT') ||
          str_contains($prog, 'LT→L0') ||
          str_contains($prog, 'DPK→L0')
      ) {
          return 'bg-emerald-50 border-emerald-200 text-emerald-700';
      }

      // stable
      if (in_array($prog, ['L0','LT','DPK','POTENSI','KL'], true)) {
  ...
  $deltaTone = function (string $key, $delta) {
      ...
      // naik = buruk
      if (in_array($key, ['lt','pct_lt','dpk','pct_dpk','potensi','pct_potensi','kl','pct_kl'], true)) {
          return $up
              ? ['text-rose-700', 'bg-rose-50 border-rose-200']
              : ['text-emerald-700', 'bg-emerald-50 border-emerald-200'];
      }
  ...
  $deltaHint = function(string $metric, $delta): string {
      ...
      if (in_array($metric, ['os','l0','rr'], true)) {
          if ($d > 0) return strtoupper($metric).' naik = membaik';
          if ($d < 0) return strtoupper($metric).' turun = memburuk';
          return 'stagnan';
      }

      if (in_array($metric, ['lt','pct_lt','dpk','pct_dpk','potensi','pct_potensi','kl','pct_kl'], true)) {
          if ($d > 0) return strtoupper($metric).' naik = memburuk';
          if ($d < 0) return strtoupper($metric).' turun = membaik';
          return 'stagnan';
      }
...
<div class="max-w-6xl mx-auto p-3 sm:p-4 space-y-4 sm:space-y-5">
  ...
  <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
      <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900">📈 Dashboard FE </h1>
...
{{-- ===========================
    FE CHART (MODEL = TLFE)
    - Scope: hanya FE yang login
    - UX: sama seperti TLFE (metric switcher + KPI strip + mode + labels + mobile)
   =========================== --}}
  ...
  <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
    ...
    <div class="flex items-center gap-2 flex-wrap justify-end">
      ...
      <div class="rounded-xl border border-slate-200 p-1 bg-slate-50 flex items-center gap-1">
        ...
        <button type="button" data-metric="os_dpk" id="btnMetricDPK"
          class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-700">
          OS DPK
        </button>
        <button type="button" data-metric="os_potensi" id="btnMetricPotensi"
                class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-700">
          OS Potensi
        </button>
        <button type="button" data-metric="os_kl" id="btnMetricKL"
                class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-700">
          OS KL
        </button>
        <button type="button" data-metric="pct_dpk" id="btnMetricPctDPK"
                class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-700">
          % DPK
        </button>
        <button type="button" data-metric="pct_potensi" id="btnMetricPctPotensi"
                class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-700">
          % Potensi
        </button>
        <button type="button" data-metric="pct_kl" id="btnMetricPctKL"
                class="px-3 py-1.5 text-xs font-semibold rounded-lg text-slate-700">
          % KL
        </button>
...
@php
  ...
  $cDpk  = (array)($cardsSrc['dpk'] ?? []);
  $cPot  = (array)($cardsSrc['potensi'] ?? []);
  $cKl   = (array)($cardsSrc['kl'] ?? []);

  $cRr   = (array)($cardsSrc['rr'] ?? []);
  $cPL   = (array)($cardsSrc['pct_lt'] ?? []);
  $cPD   = (array)($cardsSrc['pct_dpk'] ?? []);
  $cPP   = (array)($cardsSrc['pct_potensi'] ?? []);
  $cPKl  = (array)($cardsSrc['pct_kl'] ?? []);
  ...
  [$dpkText,$dpkBg] = $deltaTone('dpk', $cDpk['delta'] ?? null);
  [$potText,$potBg] = $deltaTone('potensi', $cPot['delta'] ?? null);
  [$klText,$klBg]   = $deltaTone('kl', $cKl['delta'] ?? null);

  // ==============================
  // Label Growth
  // ==============================
  $growthLabel = $mode === 'mtd'
    ? 'Growth (MtoD)'
    : 'Growth (H vs H-1)';
  ...
  $potV = (float)($cPot['value'] ?? 0);
  $klV  = (float)($cKl['value'] ?? 0);

  // ==============================
  // Base values (EOM)
  // ==============================
  $osB  = (float)($cOs['base'] ?? ($cOs['prev'] ?? 0));
  $l0B  = (float)($cL0['base'] ?? ($cL0['prev'] ?? 0));
  $ltB  = (float)($cLt['base'] ?? ($cLt['prev'] ?? 0));
  $dpkB = (float)($cDpk['base'] ?? ($cDpk['prev'] ?? 0));
  $potB = (float)($cPot['base'] ?? ($cPot['prev'] ?? 0));
  $klB  = (float)($cKl['base'] ?? ($cKl['prev'] ?? 0));
  ...
  $pctPotV = $osV > 0 ? round(($potV / $osV) * 100, 2) : null;
  $pctPotB = $osB > 0 ? round(($potB / $osB) * 100, 2) : null;

  $pctKlV = $osV > 0 ? round(($klV / $osV) * 100, 2) : null;
  $pctKlB = $osB > 0 ? round(($klB / $osB) * 100, 2) : null;

  // ==============================
  // Delta Points
  // ==============================
  $rrDeltaPts = $cRr['delta']
    ?? (!is_null($rrV) && !is_null($rrB) ? round($rrV - $rrB,2) : null);

  $pctLtDeltaPts = $cPL['delta']
    ?? (!is_null($pctLtV) && !is_null($pctLtB) ? round($pctLtV - $pctLtB,2) : null);

  $pctDpkDeltaPts = $cPD['delta']
    ?? (!is_null($pctDpkV) && !is_null($pctDpkB) ? round($pctDpkV - $pctDpkB,2) : null);

  $pctPotDeltaPts = $cPP['delta']
    ?? (!is_null($pctPotV) && !is_null($pctPotB) ? round($pctPotV - $pctPotB,2) : null);

  $pctKlDeltaPts = $cPKl['delta']
    ?? (!is_null($pctKlV) && !is_null($pctKlB) ? round($pctKlV - $pctKlB,2) : null);
  ...
  [$pdText,$pdBg] = $deltaTone('pct_dpk', $pctDpkDeltaPts);
  [$ppText,$ppBg] = $deltaTone('pct_potensi', $pctPotDeltaPts);
  [$pkText,$pkBg] = $deltaTone('pct_kl', $pctKlDeltaPts);
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-3"> 
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 flex flex-col h-full">
      ...
      <div class="mt-2">
        ...
        <div class="mt-2 flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-[11px] text-slate-500">EOM</div>
            <div class="text-[13px] sm:text-sm font-semibold text-slate-800 leading-tight">
              {{ $fmtRpFull($cPot['base'] ?? ($cPot['prev'] ?? 0)) }}
            </div>
          </div>

          <div class="text-right shrink-0">
            <div class="text-[11px] text-slate-500">%Potensi</div>
            <div class="text-[13px] sm:text-sm font-extrabold text-slate-900 leading-tight">
              {{ $fmtPct($pctPotV) }}
            </div>
            <div class="text-[10px] text-slate-400 leading-tight">
      ...
      <div class="mt-auto pt-3 space-y-2">
        ...
        <div>
          <div class="text-[11px] text-slate-500">{{ $growthLabel }} Potensi</div>
          <div class="mt-1 w-full rounded-xl border px-3 py-2 {{ $potBg }}">
            <div class="flex items-center justify-between gap-2">
              <span class="text-sm font-bold {{ $potText }} whitespace-nowrap">
                {{ $fmtDeltaRp($cPot['delta'] ?? null) }}
              </span>
              <span class="text-[11px] text-slate-500 truncate">
                {{ $deltaHint('potensi', $cPot['delta'] ?? null) }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="text-[11px] text-slate-500">{{ $growthLabel }} %Potensi</div>
          <div class="mt-1 w-full rounded-xl border px-3 py-2 {{ $ppBg }}">
            <div class="flex items-center justify-between gap-2">
              <span class="text-sm font-bold {{ $ppText }} whitespace-nowrap">
                {{ $fmtDeltaPts($pctPotDeltaPts) }}
              </span>
              <span class="text-[11px] text-slate-500 truncate">
                {{ $deltaHint('pct_potensi', $pctPotDeltaPts) }}
              </span>
            </div>
          </div>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 flex flex-col h-full">
      ...
      <div class="text-[11px] sm:text-xs text-slate-500 leading-tight">
        <div class="flex items-start justify-between gap-2">
          <span class="font-semibold text-slate-600">KL</span>
          @if($mode==='mtd')
            <span class="text-[10px] text-slate-400 text-right">{{ $baselineText }}</span>
          @endif
        </div>
      ...
      <div class="mt-2">
        ...
        <div class="mt-2 flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-[11px] text-slate-500">EOM</div>
            <div class="text-[13px] sm:text-sm font-semibold text-slate-800 leading-tight">
              {{ $fmtRpFull($cKl['base'] ?? ($cKl['prev'] ?? 0)) }}
            </div>
          </div>

          <div class="text-right shrink-0">
            <div class="text-[11px] text-slate-500">%KL</div>
            <div class="text-[13px] sm:text-sm font-extrabold text-slate-900 leading-tight">
              {{ $fmtPct($pctKlV) }}
            </div>
            <div class="text-[10px] text-slate-400 leading-tight">
      ...
      <div class="mt-auto pt-3 space-y-2">
        ...
        <div>
          <div class="text-[11px] text-slate-500">{{ $growthLabel }} KL</div>
          <div class="mt-1 w-full rounded-xl border px-3 py-2 {{ $klBg }}">
            <div class="flex items-center justify-between gap-2">
              <span class="text-sm font-bold {{ $klText }} whitespace-nowrap">
                {{ $fmtDeltaRp($cKl['delta'] ?? null) }}
              </span>
              <span class="text-[11px] text-slate-500 truncate">
                {{ $deltaHint('kl', $cKl['delta'] ?? null) }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="text-[11px] text-slate-500">{{ $growthLabel }} %KL</div>
          <div class="mt-1 w-full rounded-xl border px-3 py-2 {{ $pkBg }}">
            <div class="flex items-center justify-between gap-2">
              <span class="text-sm font-bold {{ $pkText }} whitespace-nowrap">
                {{ $fmtDeltaPts($pctKlDeltaPts) }}
              </span>
              <span class="text-[11px] text-slate-500 truncate">
                {{ $deltaHint('pct_kl', $pctKlDeltaPts) }}
              </span>
            </div>
          </div>
        </div>
...
{{-- UPDATED RISK PANEL --}}
  ...
  <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden mt-4">
    <div class="p-4 border-b border-slate-200">
      ...
      <div class="text-sm text-slate-600 mt-1">
        Fokus: akun yang memburuk dari Potensi ke KL. Total:
        <b>{{ (int)($potensiToKlNoa ?? 0) }}</b> NOA
        · OS: <b>Rp {{ number_format((int)($potensiToKlOs ?? 0),0,',','.') }}</b>
      </div>
    </div>
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse(($potensiToKl ?? []) as $r)
            ...
            <tr>
              ...
              <td class="px-3 py-2 text-center whitespace-nowrap">
                @php $isPlanned = (int)($plannedToday ?? 0) === 1; @endphp
                <button
                  type="button"
          ...
          @empty
            <tr>
              <td colspan="12" class="px-3 py-6 text-center text-slate-500">
                Tidak ada akun Potensi yang berubah menjadi KL pada periode ini ✅
              </td>
            </tr>
          @endforelse
        </tbody>
  ...
  <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden mt-4">
    <div class="p-4 border-b border-slate-200">
      <div class="font-extrabold text-slate-900 text-lg">
        DPK → Potensi (Hari Ini)
      </div>
      <div class="text-sm text-slate-600 mt-1">
        Fokus: akun yang memburuk dari DPK ke Potensi. Total:
        <b>{{ (int)($dpkToPotensiNoa ?? 0) }}</b> NOA
        · OS: <b>Rp {{ number_format((int)($dpkToPotensiOs ?? 0),0,',','.') }}</b>
      </div>
    </div>
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          @empty
            <tr>
              <td colspan="12" class="px-3 py-6 text-center text-slate-500">
                Tidak ada akun DPK yang berubah menjadi Potensi pada periode ini ✅
              </td>
            </tr>
          @endforelse
        </tbody>
        ...
        <tbody class="divide-y divide-slate-200">
          @forelse(($ltStillLt ?? []) as $r)
              ...
              $progressBadge = function($from, $to){
                if ($from === $to) return ["{$from}", 'bg-slate-50 border-slate-200 text-slate-700'];
                if ($to === 'L0')       return ["{$from}→L0", 'bg-emerald-50 border-emerald-200 text-emerald-700'];
                if ($to === 'LT')       return ["{$from}→LT", 'bg-slate-50 border-slate-200 text-slate-700'];
                if ($to === 'DPK')      return ["{$from}→DPK", 'bg-rose-50 border-rose-200 text-rose-700'];
                if ($to === 'POTENSI')  return ["{$from}→POTENSI", 'bg-rose-50 border-rose-200 text-rose-700'];
                if ($to === 'KL')       return ["{$from}→KL", 'bg-rose-50 border-rose-200 text-rose-700'];
                return ["{$from}→{$to}", 'bg-slate-50 border-slate-200 text-slate-700'];
              };
...
<script>
  ...
  // ===== Warna metric utama (scope / total) =====
  const metricPalette = {
    os_total:       { border: '#1e3a8a', fill: 'rgba(30,58,138,0.12)' },
    os_l0:          { border: '#991b1b', fill: 'rgba(153,27,27,0.12)' },
    os_lt:          { border: '#581c87', fill: 'rgba(88,28,135,0.12)' },
    os_dpk:         { border: '#b91c1c', fill: 'rgba(185,28,28,0.12)' },
    os_potensi:     { border: '#ea580c', fill: 'rgba(234,88,12,0.12)' },
    os_kl:          { border: '#7c2d12', fill: 'rgba(124,45,18,0.12)' },

    rr:             { border: '#16a34a', fill: 'rgba(22,163,74,0.45)' },
    pct_lt:         { border: '#9333ea', fill: 'rgba(147,51,234,0.45)' },
    pct_dpk:        { border: '#dc2626', fill: 'rgba(220,38,38,0.45)' },
    pct_potensi:    { border: '#f97316', fill: 'rgba(249,115,22,0.45)' },
    pct_kl:         { border: '#92400e', fill: 'rgba(146,64,14,0.45)' },
  };

  function getMetricColor(metricKey){
    return metricPalette[metricKey] || { border:'#2563eb', fill:'rgba(37,99,235,0.12)' };
  }

  // ===== Warna fix per nama FE =====
  function pickSeriesColor(label, metricKey) {
    const s = String(label || '').toLowerCase().trim();

    // total / scope utama
    if (s.includes('scope tlfe') || s.includes('scope') || s.includes('total')) {
      return getMetricColor(metricKey);
    }

    // mapping orang
    if (s.includes('aryo'))    return { border: '#dc2626', fill: 'rgba(220,38,38,0.12)' };   // merah
    if (s.includes('baried'))  return { border: '#2563eb', fill: 'rgba(37,99,235,0.12)' };   // biru
    if (s.includes('bartino')) return { border: '#16a34a', fill: 'rgba(22,163,74,0.12)' };   // hijau
    if (s.includes('harri'))   return { border: '#9333ea', fill: 'rgba(147,51,234,0.12)' };  // ungu
    if (s.includes('kholil'))  return { border: '#ea580c', fill: 'rgba(234,88,12,0.12)' };   // orange
    if (s.includes('sabani'))  return { border: '#0891b2', fill: 'rgba(8,145,178,0.12)' };   // cyan

    // fallback
    return { border: '#64748b', fill: 'rgba(100,116,139,0.12)' };
  }
  ...
  function applyDatasetVisibilityRules(datasets) {
    if (!showAllLines) {
  ...
  function buildDatasets() {
    ...
    let ds = raw.map((item, idx) => {
      const label = String(item.label || 'Metric');
      const colors = pickSeriesColor(label, metric);

      const baseRaw = Array.isArray(item.data) ? item.data : [];
      const baseFilled = carryForward(baseRaw).map(v => isNil(v) ? null : Number(v));
      ...
      return {
        ...
        // paksa warna tiap seri
        borderColor: colors.border,
        backgroundColor: colors.fill,
        pointBorderColor: colors.border,
        pointBackgroundColor: '#ffffff',

        borderWidth: idx === 0 ? 4 : 3,
        fill: false,
        pointRadius: idx === 0 ? 6 : 4,
        pointHoverRadius: idx === 0 ? 7 : 6,
        pointBorderWidth: 2,
        pointStyle: 'circle',

        // simpan warna fix utk legend / datalabel
        _lineColor: colors.border,
      };
    });

    ds = applyDatasetVisibilityRules(ds);

    console.log('buildDatasets:', ds.map(x => ({
      label: x.label,
  ...
  function getVisibleValuesForYAxis() {
    const ds = buildDatasets().filter(x => !x.hidden);
    const values = [];

    ds.forEach(item => {
      (item.data || []).forEach(v => {
        if (!isNil(v) && Number.isFinite(Number(v))) {
  ...
  function computeYScale() {
    const values = getVisibleValuesForYAxis();
    if (!values.length) {
      return {
        beginAtZero: false,
        grace: '5%',
    ...
    const pad = Math.max(range * 0.15, max * 0.01);
      ...
      options: {
        ...
        plugins: {
            ...
            labels: {
              ...
              generateLabels(chart) {
                ...
                return original.map((item) => {
                  ...
                  const c = ds._lineColor || ds.borderColor || '#334155';
                  item.strokeStyle = c;
                  item.fillStyle = c;
                  item.lineWidth = 3;
          ...
          datalabels: {
            ...
            color: function(ctx) {
              return ctx.dataset._lineColor || ctx.dataset.borderColor || '#334155';
            },
        ...
        scales: {
          ...
          y: {
            beginAtZero: initialY.beginAtZero,
            min: initialY.min,
            max: initialY.max,
            ticks: {
              display: !isMobile(),
  ...
  function refreshChart() {
    ...
    const yCfg = computeYScale();
    chart.options.scales.y.beginAtZero = yCfg.beginAtZero;
    chart.options.scales.y.min = yCfg.min;
    chart.options.scales.y.max = yCfg.max;
  ...
  function syncButtonStates() {
    ...
    setBtnActive(document.getElementById('btnMetricDPK'),        metric === 'os_dpk');
    setBtnActive(document.getElementById('btnMetricPotensi'),    metric === 'os_potensi');
    setBtnActive(document.getElementById('btnMetricKL'),         metric === 'os_kl');
    setBtnActive(document.getElementById('btnMetricPctDPK'),     metric === 'pct_dpk');
    setBtnActive(document.getElementById('btnMetricPctPotensi'), metric === 'pct_potensi');
    setBtnActive(document.getElementById('btnMetricPctKL'),      metric === 'pct_kl');
    ...
    if (btnShowAll) {
      ...
      setBtnActive(btnShowAll, showAllLines === true);
    }
  }
  ...
  function bindButtons() {
    const metricButtons = [
      ...
      ['btnMetricLT',          'os_lt'],
      ['btnMetricDPK',         'os_dpk'],
      ['btnMetricPotensi',     'os_potensi'],
      ['btnMetricKL',          'os_kl'],
      ['btnMetricRR',          'rr'],
      ['btnMetricPctLT',       'pct_lt'],
      ['btnMetricPctDPK',      'pct_dpk'],
      ['btnMetricPctPotensi',  'pct_potensi'],
      ['btnMetricPctKL',       'pct_kl'],
    ];
