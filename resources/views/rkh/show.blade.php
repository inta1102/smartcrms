...
@php
  ...
  // ===== resolve viewer role =====
  $roleValue = 'RO';
  if (isset($role) && is_string($role) && trim($role) !== '') {
  ...
  $leaderRoles = ['TLRO', 'TLFE'];
  $isReviewerLeader = in_array($roleValue, $leaderRoles, true);

  $leaderLabelMap = [
      'TLRO' => 'TLRO',
      'TLFE' => 'TLFE',
  ];
  $leaderLabel = $leaderLabelMap[$roleValue] ?? 'Atasan';

  $staffLabelMap = [
      'TLRO' => 'RO',
      'TLFE' => 'FE',
  ];
  $staffLabel = $staffLabelMap[$roleValue] ?? 'Staff';

  $isOwner = $me && ((int)$me->id === (int)$rkh->user_id);
  $statusUpper = strtoupper((string)($rkh->status ?? ''));
  $canFillLkh = ($rkh->status === 'approved');

  // viewer sebagai reviewer aktif hanya kalau:
  // - leader reviewer
  // - header masih submitted
  // - bukan dokumen miliknya sendiri
  $canReview = $isReviewerLeader
      && ($rkh->status === 'submitted')
      && ((int)$rkh->user_id !== (int)($me->id ?? 0));
@endphp
...
<div class="max-w-6xl mx-auto p-4">
  ...
  <div class="flex items-start justify-between gap-3">
    <div>
      ...
      @if(!empty($rkh->rejection_note))
        <div class="mt-2 p-3 border rounded bg-rose-50 text-sm text-rose-800 border-rose-200">
          <div class="font-semibold">Catatan Reject {{ $leaderLabel }}:</div>
    ...
    <div class="flex flex-col items-end gap-2">
        ...
        @if($isOwner && in_array($rkh->status, ['draft','rejected']))
          <form method="POST" action="{{ route('rkh.submit', $rkh->id) }}">
            ...
            <button class="px-3 py-2 rounded bg-black text-white"
              onclick="return confirm('Submit RKH ke atasan? Setelah submit, jam kegiatan tidak bisa diubah.')">
              Submit ke Atasan
            </button>
          </form>
        @endif
      ...
      {{-- ===== Panel Approval Leader Reviewer ===== --}}
      @if($canReview)
        <div class="mt-4 p-4 border rounded bg-white">
          <div class="font-bold text-slate-900">Approval {{ $leaderLabel }}</div>
          <div class="text-sm text-slate-600 mt-1">
          ...
          <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <form method="POST" action="{{ route('rkh.approve', $rkh->id) }}" class="border rounded p-3">
              ...
              <textarea name="approval_note" class="mt-2 w-full border rounded p-2 text-sm" rows="3"
                placeholder="Catatan approval (opsional). Misal: rute sudah searah, prioritas disetujui."></textarea>

              <button class="mt-3 px-4 py-2 rounded bg-emerald-600 text-white"
                onclick="return confirm('Approve RKH ini? Setelah approve, {{ strtolower($staffLabel) }} bisa mulai isi LKH.')">
                Approve
              </button>
            </form>
            ...
            <form method="POST" action="{{ route('rkh.reject', $rkh->id) }}" class="border rounded p-3">
                  ...
                  <label class="flex items-start gap-2 p-2 border-b">
                    ...
                    <div class="flex-1">
                      <div class="text-sm font-semibold">
                        ...
                        · {{ $d->nama_nasabah ?? '-' }}
                        @if(!empty($d->account_no))
                          <span class="text-slate-500">({{ $d->account_no }})</span>
                        @endif
                      </div>
              ...
              <button class="mt-3 px-4 py-2 rounded bg-rose-600 text-white"
                onclick="return confirm('Reject item yang dicentang? RKH akan kembali ke {{ strtolower($staffLabel) }} untuk revisi.')">
                Reject Selected
              </button>
            </form>
          </div>
  ...
  <div class="mt-4 border rounded overflow-auto">
    <table class="min-w-full text-sm">
      ...
      <tbody>
          ...
          <tr class="border-t">
            ...
            <td class="p-2">
              @if($hasLkhFilled)
                ...
                @if($visitDone && empty($d->lkh))
                  <div class="text-[11px] text-slate-500 mt-1">Sumber: Visit DONE</div>
                @endif
              @else
            ...
            <td class="p-2 whitespace-nowrap">
              ...
              @elseif($visitDone)
                <a class="underline text-msa-blue" href="{{ route('rkh.details.visitStart', $d->id) }}">Lihat LKH</a>
                <div class="text-[11px] text-slate-500 mt-1">Visit sudah selesai</div>

              @else
                @if(!$canFillLkh)
                  <button type="button"
                          class="text-slate-400 cursor-not-allowed underline"
                          onclick="alert('LKH hanya bisa diisi setelah RKH di-APPROVE atasan. Status saat ini: {{ strtoupper($rkh->status) }}');">
                    Isi LKH
                  </button>
        ...
        @empty
          <tr>
            <td class="p-4 text-slate-500" colspan="8">Belum ada kegiatan.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
