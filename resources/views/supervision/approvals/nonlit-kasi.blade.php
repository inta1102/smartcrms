...
>
  <div>
    <h1 class="text-xl font-bold">Approval Non-Litigasi (KASI)</h1>
      ...
      <tbody>
        @forelse ($items as $nonLit)
          @php
            ...
            $approveUrl = route('supervision.kasi.approvals.nonlit.approve', $nonLit);
            $rejectUrl  = route('supervision.kasi.approvals.nonlit.reject',  $nonLit);
          @endphp
        ...
        @empty
          <tr>
            <td colspan="5" class="px-4 py-8 text-center text-slate-500">
              Tidak ada usulan non-litigasi menunggu approval KASI.
            </td>
          </tr>
        @endforelse
      </tbody>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
      <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl border border-slate-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
          <div class="text-sm font-bold text-slate-900">Approve Non-Litigasi (KASI)</div>
        ...
        <form method="POST" :action="approveUrl" class="p-5 space-y-4">
          ...
          <div>
            <label class="text-xs font-semibold text-slate-700">Catatan Approval (opsional)</label>
            <textarea name="approval_notes" rows="3"
                      class="mt-1 w-full rounded-xl border border-slate-200 p-2 text-sm"
                      placeholder="misal: disetujui, syarat monitoring, dll"></textarea>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label class="text-xs font-semibold text-slate-700">Effective Date (opsional)</label>
              <input type="date" name="effective_date"
                     class="mt-1 w-full rounded-xl border border-slate-200 p-2 text-sm">
            </div>
            <div>
              <label class="text-xs font-semibold text-slate-700">Monitoring Next Due (opsional)</label>
              <input type="date" name="monitoring_next_due"
                     class="mt-1 w-full rounded-xl border border-slate-200 p-2 text-sm">
              <div class="mt-1 text-[11px] text-slate-400">
                Jika diisi, sistem akan membuat agenda monitoring otomatis.
              </div>
            </div>
          </div>
  ...
  <div x-show="openReject" x-cloak class="fixed inset-0 z-50">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
      <div class="w-full max-w-lg rounded-2xl bg-white shadow-xl border border-slate-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
          <div class="text-sm font-bold text-rose-700">Reject Non-Litigasi (KASI)</div>
          <div class="text-xs text-slate-500">
            Debitur: <span class="font-semibold" x-text="selectedName"></span>
          </div>
        </div>
        ...
        <form method="POST" :action="rejectUrl" class="p-5 space-y-4">
          ...
          <div>
            ...
            <textarea name="reason" rows="4" required
                      class="mt-1 w-full rounded-xl border border-slate-200 p-2 text-sm"
                      placeholder="misal: strategi belum tepat, data tidak valid, komitmen belum realistis, dll"></textarea>
          </div>
