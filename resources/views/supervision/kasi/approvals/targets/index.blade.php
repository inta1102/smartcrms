...
@section('content')
@php
    // ✅ kompatibel dengan controller lama/baru
    $targets = $rows ?? $targets ?? collect();
    ...
    $badge = function(string $status) {
        ...
        return match($s) {
            'inbox'       => 'bg-slate-50 text-slate-700 border-slate-200',
    ...
    // SLA config (controller sudah supply $kasiSlaDays)
    $kasiSlaDays = $kasiSlaDays ?? (int) config('crms.sla.kasi_days', 2);

    $isOverSla = function($createdAt) use ($kasiSlaDays) {
        try {
            return \Illuminate\Support\Carbon::parse($createdAt)->lt(now()->subDays($kasiSlaDays));
        } catch (\Throwable $e) {
            return false;
        }
    };

    $qs = fn($key, $default = '') => request()->query($key, $default);
    ...
    $chip = fn($text, $tone='slate') => match($tone) {
    ...
    // Dropdown status options (sesuai controller)
    $statusOptions = [
        'inbox'        => 'Inbox (Pending KASI + Pending TL skip)',
        'pending_kasi' => 'Pending KASI',
        'pending_tl'   => 'Pending TL',
        'active'       => 'Active',
        'rejected'     => 'Rejected',
...
>
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-slate-900">Approval Target (KASI)</h1>
        <p class="mt-1 text-slate-500">
            Inbox berisi: <b>Pending KASI</b> + <b>Pending TL (skip TL)</b> untuk AO yang tidak punya TL.
        </p>
    </div>
    ...
    <form class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm mb-6" method="get">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
            <div class="lg:col-span-3">
                ...
                <select name="status" class="{{ $selectBase }}">
                    @foreach($statusOptions as $st => $text)
                        <option value="{{ $st }}" @selected(($qs('status','inbox')) === $st)>{{ $text }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="lg:col-span-5">
                ...
                <input name="q" value="{{ $qs('q','') }}" class="{{ $inputBase }}" placeholder="misal: SUROJO / aditya / rs / lelang...">
            </div>
            ...
            <div class="lg:col-span-2">
                ...
                <select name="per_page" class="{{ $selectBase }}">
                    @foreach([15,25,50] as $pp)
                        <option value="{{ $pp }}" @selected((int)$qs('per_page',15) === $pp)>{{ $pp }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="flex flex-col gap-2 lg:items-end">
                    ...
                    <div class="flex gap-2 w-full lg:w-auto lg:justify-end">
                        <a href="{{ route('supervision.kasi.approvals.targets.index') }}" class="{{ $btnGhost }} w-1/2 lg:w-auto">Reset</a>
            ...
            <table class="min-w-full text-sm">
                ...
                @forelse($targets as $t)
                    @php
                        ...
                        $needsTl = (int)($t->needs_tl_approval ?? 1);

                        // ✅ Kasi boleh aksi untuk:
                        // - pending_kasi
                        // - pending_tl tetapi skip TL (needs_tl_approval=0)
                        $canAct = ($st === \App\Models\CaseResolutionTarget::STATUS_PENDING_KASI)
                            || ($st === \App\Models\CaseResolutionTarget::STATUS_PENDING_TL && $needsTl === 0);
                        ...
                        $payload = [
                            ...
                            'target_date' => $t->target_date?->format('d M Y') ?? '-',
                            'target_outcome' => (string) ($t->target_outcome ?? '-'),
                            'needs_tl_approval' => $needsTl,
                            'detail_url' => route('cases.show', $t->npl_case_id),
                            'approve_url' => route('supervision.kasi.approvals.targets.approve', $t),
                            'reject_url' => route('supervision.kasi.approvals.targets.reject', $t),
                        ];
                    @endphp
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 font-semibold text-slate-900">
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                @if($over)
                                    <span class="{{ $chip('Over SLA','rose') }}">Over SLA</span>
                                @endif

                                @if($st === \App\Models\CaseResolutionTarget::STATUS_PENDING_TL && $needsTl === 0)
                                    <span class="{{ $chip('Skip TL','amber') }}">Skip TL</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 text-slate-700">
                            <div class="font-semibold">{{ $t->target_date?->format('d M Y') ?? '-' }}</div>
                        ...
                        <td class="px-4 py-3 text-slate-600">
                            ...
                            <div class="text-xs text-slate-400">SLA: {{ $kasiSlaDays }} hari</div>
                        </td>
        ...
        <div class="relative mx-auto w-full max-w-2xl px-4 py-8">
            <div class="rounded-2xl bg-white shadow-xl border border-slate-200 overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
                    <div>
                        <div class="text-sm text-slate-500" x-text="mode==='detail' ? 'Detail Target' : (mode==='approve' ? 'Approve Target' : 'Reject Target')"></div>
                        ...
                        <div class="flex items-center justify-between gap-3 flex-wrap">
                            <div class="text-sm font-semibold text-slate-900">
                                ...
                                <span class="ml-1 inline-flex items-center rounded-lg border px-2 py-1 text-xs font-semibold"
                                      :class="(payload.status ?? '') === 'pending_kasi'
                                                ? 'border-rose-200 bg-rose-50 text-rose-700'
                                                : ((payload.status ?? '') === 'pending_tl'
                                ...
                                <template x-if="(payload.status ?? '')==='pending_tl' && (payload.needs_tl_approval ?? 1)===0">
                                    <span class="ml-2 inline-flex items-center rounded-lg border border-amber-200 bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-800">
                                        Skip TL
                                    </span>
                                </template>
                            </div>
                    ...
                    <template x-if="mode==='approve'">
                        <form method="post" :action="payload.approve_url" class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows="3" class="{{ $inputBase }}" placeholder="misal: setujui dengan catatan follow-up dokumen / update status debitur..."></textarea>
                                <div class="mt-1 text-xs text-slate-500">
                                    Approve akan mengaktifkan target dan auto-create agenda follow-up.
                                </div>
                            </div>
                        ...
                        <form method="post" :action="payload.reject_url" class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="reason" rows="4" class="{{ $inputBase }}" required
                                          placeholder="Tuliskan alasan jelas (audit-friendly). Contoh: Strategi belum sesuai; minta revisi RS + bukti komunikasi + rencana visit..."></textarea>
                                <div class="mt-1 text-xs text-slate-500">
